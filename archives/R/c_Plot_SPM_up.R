...
lapply(c("tidyr","dplyr","stringr","ggplot2","Hmisc","xlsx","car","data.table","grid"), require, character.only = T)

  # Import des données brutes
data_SPM <- read.xlsx("Y:/Data/Epaule_manutention/Hommes-Femmes/Résultats/RAW_0D/Zones_SPM_up.xlsx",sheetName="Zones_SPM")

# Assignation des facteurs : Poids et Hauteur
data_SPM["Poids"] <- factor(data_SPM[["Poids"]],labels=c("12kg-6kg","18kg-12kg"))
data_SPM["Hauteur"] <- factor(data_SPM[["Hauteur"]],labels=c("hips-shoulders","hips-eyes","shoulders-eyes"))
...
p <- p + scale_x_continuous(name = "normalized time (% of trial)",limits = c(0, 100), breaks = c(0,20,40,60,80,100))
  # Axe y
p <- p + ylab("muscle")
  # Subplot
...
p <- p + scale_colour_gradient2("mean difference\n(% MVC)",limits=c(-25, 25), low="deepskyblue2", mid="white",high="firebrick3")
p
...
save(p, file =  "SPM_gantt_up.RData")
ggsave("SPM_gantt_up.pdf", height = 15, width = 20, units='in', dpi=600)
