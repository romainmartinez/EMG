...
data_mean <- subset(data_mean,!(is.na(data_mean["Comp"])))
  ...
  # Essais de montées
data_spider_up <- dplyr::filter(data_mean, Hauteur == "hips shoulders" | Hauteur == "hips eyes" | Hauteur == "shoulders eyes")

  # Essais de descente
data_spider_down <- dplyr::filter(data_mean, Hauteur == "shoulders hips" | Hauteur == "eyes hips" | Hauteur == "eyes shoulders")
...
data_spider_down$signi <- signi_down[,"Signi"]
  ...
  # Plot Radar
    # AES
    radarplot <- ggplot(data_spider_down, aes(x = Muscle)) +
    # Polygone 
      geom_polygon(aes(group = Sexe, y = mean, fill = Sexe, color = Sexe), alpha=0.3, size = 1, show.legend = TRUE) +
      geom_polygon(aes(group = Sexe, y = max, color = Sexe), linetype = "dashed", size = 0.8, show.legend = TRUE, fill = NA) +
    # Theme  
      RadarTheme +
    # Noms de la légende  
      labs(col = "max",fill = "mean") +
    # Échelle  
      xlab("") + ylab("") +scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, 20)) +
    # Obtenir les coordonnées  
      coord_radar() +
    # Texte  
      geom_text(data = data_spider_down, aes(x = Muscle, y = 90, label = signi), size = 6) +
    # Subplot  
      facet_grid(Comp ~ Hauteur) +
    # Label de l'axe y
      ylab("EMG envelope (% MVC)") +
    ...
    # Définition des couleur
      scale_colour_manual(values = c("Men" = "deepskyblue2","Women" =  "firebrick3")) +
...
save(radarplot, file =  "Radarplot_meanRMS_down.RData")
ggsave("Radarplot_meanRMS_down.pdf", height = 15, width = 20, units='in', dpi=600)
  ###########################
